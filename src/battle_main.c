...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(MEGATRAINER, "MEGA TRAINER"),
};
...
static void BattleStartClearSetData(void)
{
    ...
    gBattleStruct->swapDamageCategory = FALSE; // Photon Geyser, Shell Side Arm, Light That Burns the Sky, Hidden Power.
...
s8 GetMovePriority(u32 battler, u16 move)
{
    ...
    else if (ability == ABILITY_HELLISH_SOUL
        && (B_GALE_WINGS < GEN_7 || BATTLER_MAX_HP(battler))
...
static void HandleEndTurn_BattleWon(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !(gBattleTypeFlags & BATTLE_TYPE_LINK))
    {
        ...
        case TRAINER_CLASS_MAGMA_LEADER:
            ...
            break;
        case TRAINER_CLASS_MEGATRAINER:
...
static void HandleEndTurn_FinishBattle(void)
{
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        FadeOutMapMusic(5);
        if (B_TRAINERS_KNOCK_OFF_ITEMS == TRUE || B_RESTORE_HELD_ITEMS == TRUE)
            TryRestoreHeldItems();
